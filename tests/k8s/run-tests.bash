...
function reinstall_kubeadmn(){
    vm="${1}"
    k8s_version="${2}"
    vmssh ${vm} '/home/vagrant/go/src/github.com/cilium/cilium/tests/k8s/cluster/cluster-manager.bash reinstall --reinstall-kubeadm'
...
function run_tests(){
    ...
    # Run the GSG first and then restart the cluster to run the remaining tests
    #vagrant ssh ${node1} -- -o SendEnv=k8s_version ${k8s_version} -t 'set -e; /home/vagrant/go/src/github.com/cilium/cilium/tests/k8s/tests/00-gsg-test.bash'
    #
    # Set up cilium-lb-ds and cilium-ds
    #deploy_cilium ${k8s_version}
...
reinstall_kubeadmn ${node1} "1.7.0-00"
reinstall_kubeadmn ${node2} "1.7.0-00"
run_tests "1.7.0-00"
